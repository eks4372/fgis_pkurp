...
try:
    for index, row in df.iterrows():
        ...
        def check_delete():
            try:
                x = browser.find_element(By.CSS_SELECTOR, '#bs-tabs-registry_record_nav-underlying_documents .glyphicon-remove-circle')
        ...
        print('далее')
        while browser.find_element(By.CLASS_NAME, 'btn-next').is_enabled():
            print('кнопка далее не активна')
            sleep(0.3)
        # browser.find_element(By.LINK_TEXT, 'Далее').click()
        sleep(1)

        # radios = browser.find_elements(By.CSS_SELECTOR, '.form-group .radio_buttons')
        # for r in radios:
        #     if 'Внести сведения в ЕГРН' == r.text:
        #         r.click()
        try:
            element = WebDriverWait(browser, 10).until(
